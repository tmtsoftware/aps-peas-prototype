...
import org.tmt.peasproceduredataservice.TestHelper
import org.tmt.peasproceduredataservice.impl.PeasProcedureDataServiceImpl
import org.tmt.peasproceduredataservice.core.models.{AdminGreetResponse, GreetResponse, UserInfo}

import scala.concurrent.Future
import scala.util.Random

class PeasProcedureDataServiceRouteTest extends AnyWordSpec with ScalatestRouteTest with PekkoHttpCompat with BeforeAndAfterEach with HttpCodecs {

  private val service1: PeasProcedureDataServiceImpl                   = mock[PeasProcedureDataServiceImpl]
  private val service2                               = mock[JPeasProcedureDataServiceImplWrapper]
  ...
  private val route: Route = new PeasProcedureDataServiceRoute(service1, service2, securityDirectives).route

  override protected def beforeEach(): Unit = reset(service1, securityDirectives)

  "PeasProcedureDataServiceRoute" must {
  ...
  val connection: Connection.HttpConnection = HttpConnection(ComponentId(Prefix(TestHelper.randomSubsystem, "peasproceduredataservice"), Service))
}
