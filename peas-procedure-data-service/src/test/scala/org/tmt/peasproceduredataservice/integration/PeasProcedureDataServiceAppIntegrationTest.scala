package org.tmt.peasproceduredataservice.integration
...
import org.tmt.peasproceduredataservice.impl.PeasProcedureDataServiceWiring
import org.tmt.peasproceduredataservice.core.models.{AdminGreetResponse, GreetResponse, UserInfo}
import org.tmt.peasproceduredataservice.http.HttpCodecs

import scala.compiletime.uninitialized
import scala.concurrent.duration.DurationInt
import scala.concurrent.Await

class PeasProcedureDataServiceAppIntegrationTest extends ScalaTestFrameworkTestKit with AnyWordSpecLike with Matchers with HttpCodecs {
  ...
  val peasproceduredataserviceAppPort                    = 8085
  val peasproceduredataserviceWiring                     = new PeasProcedureDataServiceWiring(Some(peasproceduredataserviceAppPort))
  val appConnection: HttpConnection    = peasproceduredataserviceWiring.settings.httpConnection
  ...
  protected override def beforeAll(): Unit = {
    ...
    peasproceduredataserviceWiring.start(Metadata.empty).futureValue
  ...
  protected override def afterAll(): Unit = {
    ...
    peasproceduredataserviceWiring.stop().futureValue
    super.afterAll()
  }

  "PeasProcedureDataServiceWiring" must {

    "start the peasproceduredataservice app and register with location service" in {
